...
# import copy
            # print("\n -------------------------- ")
            # print("> on GPU\n")
            # print(f"expert C2G     {t33-t22} - {round((t33-t22)/(t11-t00),2)} Units")
            # print(f"expert forward {(t11-t00)} - {1} Units")
            # print(f"total          {(t33-t22)+(t11-t00)} - {round(((t33-t22)+(t11-t00))/(t11-t00),2)} Units\n")
            # print("> on CPU")
            # print(f"tokens G2C     {t2-t1} - {round((t2-t1)/(t11-t00),2)} Units")
            # print(f"expert forward {t4-t3} - {round((t4-t3)/(t11-t00),2)} Units")
            # print(f"outputs C2G    {t6-t5} - {round((t6-t5)/(t11-t00),2)} Units")
            # print(f"total          {(t4-t1)+(t6-t5)} - {round(((t4-t1)+(t6-t5))/(t11-t00),2)} Units")
    ...
    def __init__(self, config,
        ...
        if args.num_experts_switch is not None:
            ...
            if self.num_experts <= 1: # dense, not MoE
                ...
                # print("############################")
                # print(f"args.moe_expert_parallel_size = {args.moe_expert_parallel_size}")
                # print(f"args.topk = {args.topk}")
                # print(f"args.moe_train_capacity_factor = {args.moe_train_capacity_factor}")
                # print(f"args.moe_eval_capacity_factor = {args.moe_eval_capacity_factor}")
                # print(f"args.moe_min_capacity = {args.moe_min_capacity}")
                # print(f"args.moe_token_dropping = {args.moe_token_dropping}")
                # print(f"args.use_tutel = {args.use_tutel}")
                # print(f"enable_expert_tensor_parallelism = {enable_expert_tensor_parallelism}")
                # print("############################")
                # ---- test FMoE ---
                # self.mlp = MyMoE(self.num_experts / 4, 768, 768*4, 1, torch.nn.functional.gelu)
...
class MyMoE(FMoE):
    ...
    def forward(self, inp):
        moe_outp = super().forward(inp)
        return moe_outp  # maybe return output, self.deepspeed_moe.l_aux, self.deepspeed_moe.exp_counts?